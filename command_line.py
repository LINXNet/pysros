...
def parse_and_get_args():
  ...
  if any(action in args.actions for action in
         ['diff', 'replace', 'merge']) and not args.candidate_file_path:
    parser.error(
      "diff, replace and merge actions requires -c, --candidate-file-path.")
  return args
...
with driver(hostname=switch_name,
  ...
  if operation == 'running':
    result = device.get_config(retrieve='running', optional_args={"format": "cli"})
